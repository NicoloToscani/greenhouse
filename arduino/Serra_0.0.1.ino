...
void loop() //OB1
{
  ...
  if ((err = dht11.read(&temperature, &humidity, NULL)) != SimpleDHTErrSuccess) {
    ...
    /*
    Serial.print("SERVO: ");
    Serial.println(Servo_Sx_cmd);
    Serial.print("RESULT: ");
    Serial.println(Result_Servo);
    */
    Serial.print("\n\n");
    // delay(4000);
 ...
 // Serial.print("\nLOOP SERVO MOTORI:\n");
  //Serial.print("Servo DX loop command: ");
  //Serial.println(Servo_Sx_cmd);
  ...
  /*
              // Motor dx open
              if(Servo_Dx_cmd == 5){
                  for (pos = 5; pos < 180; pos += 1) // imposta un ciclo con valori che vanno da 0 a 180, sarano i gradi di spostamento del nostro servo
                  {
                    servo_dx.write(Servo_Dx_cmd);              // con il metodo write() passi all'oggetto myservo la posizione che deve raggiungere,
              ...
              /*
              for (pos = 180; pos > 0; pos -= 1) // imposta un ciclo con valori che vanno da 0 a 180, sarano i gradi di spostamento del nostro servo
                {
                  servo.write(pos);              // con il metodo write() passi all'oggetto myservo la posizione che deve raggiungere,
                  // il servo si sposterà gradualmente dalla sua posizione 0° alla posizione 180°
                  delay(15);                       // imposta un ritardo di 15 millesimi di secondo per ogni ciclo del for.
                  // Più sarà alto il ritardo più il servo sarà lento.
                  Serial.print("Servo: ");
...
// delay(2000);
  ...
  int Result_Servo = 0;
   ...
   Result_Servo = Client.ReadArea(S7AreaDB, // We are requesting DB access
                                     2,        // DB Number = 1
                                     8,        // Start from byte N.0
                                     sizeof(int),
                                     &MyBuffer);  // Put them into our target (Buffer or PDU)

    Servo_Sx_cmd = S7.IntegerAt(&MyBuffer, 0);
...
/*
  int Result_Servo = Client.ReadArea(S7AreaDB, // We are requesting DB access
                                     2,        // DB Number = 1
                                     8,        // Start from byte N.0
                                     2,     // We need "Size" bytes
                                     &MyBuffer);  // Put them into our target (Buffer or PDU)

  Servo_Dx_cmd = S7.IntegerAt(&MyBuffer, 0);
  Serial.print("Servo_Dx_cmd: ");
  Serial.print(Servo_Dx_cmd);
  servo_sx.write(Servo_Dx_cmd);
  */
  /*
